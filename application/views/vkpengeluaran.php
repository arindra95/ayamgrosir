			...
			<nav class="navbar navbar-expand-sm " style="background-color: #685AB4" >
				...
				<div class="collapse navbar-collapse" id= "myNavbar">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item"><a class="nav-link text-white" href="<?php echo base_url()."kasir/pengeluaran" ?>">Pengeluaran</a></li>
	...
	<div class="container-fluid">
		<div class="card card-default " id="form-edit" style="display: none">
			<div class="card-header">
				<h3 id="tittle" class="card-tittle pull-left">Tambah data pengeluaranu</h3>
					...
					<div class="row h-75 justify-content-center">
						<div class="col-sm-5 col-sm-offset-1" >
							...
							<div class="form-group row">
								<label for="id" class="col-sm-3 col-form-label text-sm-right text-left" style="">Tanggal</label>
								<div class="col-sm-9">
									<input type="text" id="tgl" name="tgl" class="form-control" required>
									<input type="hidden" id="nopeng" name="nopeng" class="form-control" required>
									<small class="help-block"></small>
								</div>
							</div>
							<div class="form-group row" >
								<label for="jenis" class="col-form-label col-sm-3 text-sm-right text-left">Jenis Pengeluaran</label>
								<div class="col-sm-9">
									<input id="jenis" value="" class="form-control" required="">
									<small class="help-block"></small>
								</div>
							</div>
							<div class="form-group row" >
								<label for="total" class="col-form-label col-sm-3 text-sm-right text-left">Total</label>
								<div class="col-sm-9">
									<input id="total" value="" class="form-control" required="">
									<small class="help-block"></small>
								</div>
							</div>
						</div>
		...
		<div class="card card-default">
			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="pengtable" class="table table-hover" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th width="0">Id</th>
								<th width="0">Tanggal</th>
								<th width="0">Jenis Pengeluaran</th>
								<th width="0">Total</th>
							</tr>
						</thead>
						<tbody >
...
<script>
	...
	$(document).ready(function(){
		...
		$('pengtable td').css('white-space','initial');
		var table = $("#pengtable").DataTable({

			"ajax": {
				"url" : "<?php echo base_url().'pengeluaran/load_pengeluaran'?>",
		...
		$('#pengtable tbody').on( 'click', 'tr', function(){
    		$("#pengtable tbody tr").removeClass('selected');
		...
		btnCreate.on('click', function(){
		 	//save_method = 'add';
		 	$("#tittle").text("Tambah data pengeluaran");
		 	resetForm();
			data=null;
			openForm();
    	...
    	//closeForm();
    		//save_method = 'update';
    		$("#tittle").text("Ubah data pengeluaran");
    		nopeng.val(data.nopengeluaran)
    		...
    		openForm();
			url = '<?php echo base_url().'pengeluaran/perbarui/'?>'+data.nopengeluaran;
		});



		btnDelete.on('click', function(){
			if(data.nopengeluaran != ''){
				...
				if(hapus){
					...
					$.ajax({
						...
						type: "POST",
						data: "nopengeluaran="+data.nopengeluaran,
						beforeSend:function(){
